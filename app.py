...
st.markdown("""
<style>
.main-title { font-size: 3.5rem; font-weight: 800; color: white; margin-bottom: 0px; }
.sub-title { font-size: 1.5rem; color: #94a3b8; font-weight: 400; margin-bottom: 2.5rem; }
.stTabs [data-baseweb="tab-list"] { gap: 2rem; border-bottom: 1px solid #334155; }
.stTabs [data-baseweb="tab"] { font-size: 1.25rem; font-weight: 600; padding: 1rem 0; color: #94a3b8; }
...
with tab1:
    ...
    try:
        st.image("assets/sample_predictions.png", caption="Ground Truth vs Highest Confidence Output Probabilities", use_column_width=True)
    except Exception:
        st.warning("Visualization syncing...")
...
with tab2:
    ...
    st.markdown("---")
    try:
        st.image("assets/training_curve.png", caption="Feature Extraction Convergence Trajectory", use_column_width=True)
        
        c1, c2 = st.columns(2)
        with c1:
            st.image("assets/roc_curves.png", caption="Baseline Receiver Operating Characteristic (14 Distinct Pathologies)", use_column_width=True)
        with c2:
            st.image("assets/auc_bar.png", caption="Per-Class Area Under Curve Distribution (Global Mean: 0.8378)", use_column_width=True)
    except Exception:
        pass
...
with tab3:
    ...
    try:
        ...
        with col1:
            st.image("assets/chexpert_generalization_auc.png", caption="Stanford CheXpert Shared Assessment (Zero-Shot Mean AUC: 0.8414)", use_column_width=True)
        with col2:
            st.image("assets/chexpert_roc_curves.png", caption="Generalization ROC Trajectories Tracking the 5 Overlapping Pathologies", use_column_width=True)
    except Exception:
        pass
